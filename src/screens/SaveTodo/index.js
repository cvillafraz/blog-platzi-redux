...
const Add = ({
	...
	const { items: todos, userId, title, loading, error, redirect } = useSelector(
		state => state.todos
	)
	const dispatch = useDispatch()
	const store = useStore()
	const changeInput = useCallback(
		(name, val) => todosActions.changeInput(name, val, store),
		[store]
	)
	const addTodo = useCallback(
		newTodo => todosActions.addTodo(newTodo, dispatch),
		[dispatch]
	)
	const editTodo = useCallback(
		editedTodo => todosActions.editTodo(editedTodo, store),
		[store]
	)
	useEffect(() => {
		if (user_id && todo_id) {
	...
	function showAction() {
		if (redirect) return <Redirect to="/blog-platzi-redux/tareas" />
	...
	return (
		<form>
			...
			<button disabled={disable()} onClick={save} className="btn btn-info">
				Guardar
			</button>
			{showAction()}
		</form>
