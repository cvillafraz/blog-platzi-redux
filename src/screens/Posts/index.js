...
import { showComments, getComments } from '../../config/actions/posts/showComments'
const Posts = ({ match }) => {
    ...
    const toggleComments = useCallback((postKey, comKey) => showComments(postKey, comKey, store), [store])
    const commentsCall = useCallback((postKey, comKey) => getComments(postKey, comKey, store), [store])
    useEffect(() => {
    ...
    useEffect(() => {
        async function getPosts() {
            if (users.items.length && !('posts_key' in users.items[match.params.key])) {
    ...
    function showUserName() {
        if (users.error) return <Fatal message={users.error} />
            ...
            return <h1 data-testid="username" className="mb-4 display-4">{`Publicaciones de ${users.items[match.params.key].name}`}</h1>
        }
        else return
    }
    function showPosts() {
        ...
        if (!('posts_key' in users.items[match.params.key])) return;
        const { posts_key } = users.items[match.params.key];
        if (posts.items.length > 0) return posts.items[posts_key].map((post, index) => (
            <div
                key={post.id}
                className="card my-4"
            >
                <h2 data-testid={post.id == 1 && 'post-title'} className="card-title card-header">
                    {post.title}
                </h2>
                <div className="card-body">
                    <p data-testid={post.id == 1 && 'post-body'}>
                        {post.body}
                    </p>
                    <button
