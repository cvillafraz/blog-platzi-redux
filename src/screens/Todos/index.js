...
const Todos = () => {
	...
	const callTodos = useCallback(() => todosActions.fetchTodos(dispatch), [
		dispatch
	])
	const deleteTodo = useCallback(
		todoId => todosActions.deleteTodo(todoId, dispatch),
		[dispatch]
	)
	const changeCheck = useCallback(
		(userId, todoId) => todosActions.changeCheck(userId, todoId, store),
		[store]
	)
	useEffect(() => {
	...
	function showTodos(userId) {
		...
		return Object.keys(perUser).map(todoId => (
			...
			>
				<p style={{ fontSize: '1.2rem', flexBasis: '55%' }} className="m-0">
					<input
						type="checkbox"
				...
				<div>
					<button className="btn btn-info">
						<Link
							...
							to={`/blog-platzi-redux/tareas/guardar/${userId}/${todoId}`}
						>
							Editar
						</Link>
					</button>
		...
		<main>
			...
			{Object.keys(todos).map(userId => (
				<div key={userId}>
					...
					<ul className="list-group">{showTodos(userId)}</ul>
				</div>
			))}
		</main>
	)
